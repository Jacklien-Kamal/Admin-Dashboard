          ...
          <div className="hidden  border  rounded-xl shadow lg:block overflow-auto">
            <table className="min-w-full table-fixed bg-white dark:bg-gray-600">
              <thead className="bg-primary-light dark:bg-primary-dark text-white">
                <tr>
                  <th
                    className="px-3 py-1  text-center w-12 cursor-pointer"
                    onClick={() => toggleSort("id")}
                  >
                    ID
                  </th>
                  <th
                    className="px-3 py-2 text-left w-64 cursor-pointer"
                    onClick={() => toggleSort("name")}
                  >
                    User
                  </th>
                  <th className="px-3 py-1  text-center w-64">Phone</th>
                  <th
                    className="px-3 py-1  text-center w-64 cursor-pointer"
                    onClick={() => toggleSort("email")}
                  >
                    Email
                  </th>
                  <th
                    className="px-3 py-1  text-center w-32 cursor-pointer"
                    onClick={() => toggleSort("status")}
                  >
                    Status
                  </th>
                  <th className="px-3 py-1  text-center w-64">
                    Meetings Attended
                  </th>
                  <th className=" px-3 py-1    text-center w-40">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-100 transition">
                    ...
                    <td className=" relative p-2 border text-center">
                      <span
                      ...
                      <span
                        className={`absolute bottom-7 left-10 w-2 h-2 rounded-full${
                          user.status === "active"
                            ?  " bg-green-600"
                            : " bg-red-700"
                    ...
                    <td className="p-2 border text-center">
                      <div className="flex flex-col items-center gap-1">
                        <span className="text-xs font-medium text-gray-700 dark:text-white">
                          {user.meetings.filter((m) => m.attended).length} /{" "}
                    ...
                    <td className="p-2 border text-center space-x-2">
                      <button
                        className="bg-indigo-100 hover:bg-indigo-300 text-indigo-600 px-2 py-1 rounded shadow"
                        onClick={() => handleEdit(user)}
                      >
                        <MdOutlineModeEdit />{" "}
                      </button>
                      <button
                        className="bg-red-200 hover:bg-red-300 text-red-600 px-2 py-1 rounded shadow"
                        onClick={() => {
          ...
          <div className="block md:hidden space-y-4">
            {filteredUsers.map((user) => (
              <div key={user.id} className="bg-white p-4 rounded shadow">
                ...
                <button
                  className="mt-3 text-red-700 hover:underline"
      ...
      {isModalOpen && selectedUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-[600px] max-h-[90vh] overflow-y-auto space-y-4 shadow-lg">
            ...
            <label className="block font-medium mb ">
              {selectedUser.name} Meetings
            </label>
            <select
              className="w-full border border-gray-300 p-2 rounded-md"
              name="meetingId"
            >
              {selectedUser.meetings
                .filter((meeting) => meeting.attended === true)
                .map((meeting) => (
